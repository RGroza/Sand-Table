...
def run_MRot(num_steps, delay):
    ...
    if num_steps > 0:
        M_Rot.TurnStep_test(Dir='forward', steps=num_steps, stepdelay = delay)
    else:
        M_Rot.TurnStep_test(Dir='backward', steps=abs(num_steps), stepdelay = delay)
    M_Rot.Stop()

def run_MLin(num_steps, delay):
    M_Lin.SetMicroStep('software','fullstep')
    if num_steps > 0:
        M_Lin.TurnStep_test(Dir='forward', steps=num_steps, stepdelay = delay)
    else:
        M_Lin.TurnStep_test(Dir='backward', steps=abs(num_steps), stepdelay = delay)
    M_Lin.Stop()
